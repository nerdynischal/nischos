...
**Remediation status (17 September 2026): Fixed locally.** Copy feedback uses a persistent `role="status"` region outside the hidden icon wrapper. Following the 18 September visual refinement, success uses the original compact toast for five seconds with a separate visually hidden status announcement; errors remain visible. Repeated attempts replace the message content, and rejected or unavailable clipboard access exposes an error and a labelled read-only field for manual copying. Focus stays on the initiating button; tabbing to the fallback selects its value. A successful retry removes the fallback. Duplicate in-flight requests are ignored without disabling the focused button. Five regression tests cover these paths, including the toast timer, repeated-copy timer reset, and retained announcement and focus. All 130 tests and the full production check pass. A 320px browser fixture with a simulated clipboard verified failure, manual selection, retry, success, and focus retention without changing the user's clipboard. Actual screen-reader speech testing remains outstanding. Evidence below records the original defect.
